...
$parse['who']    = "<option value=\"1\"". (($who == "1") ? " SELECTED" : "") .">Jugador</option>";
$parse['who']   .= "<option value=\"2\"". (($who == "2") ? " SELECTED" : "") .">Alianza</option>";

$parse['type']   = "<option value=\"1\"". (($type == "1") ? " SELECTED" : "") .">Puntos</option>";
$parse['type']  .= "<option value=\"2\"". (($type == "2") ? " SELECTED" : "") .">Flotas</option>";
$parse['type']  .= "<option value=\"3\"". (($type == "3") ? " SELECTED" : "") .">Investigación</option>";
$parse['type']  .= "<option value=\"4\"". (($type == "4") ? " SELECTED" : "") .">Edificios</option>";
...
else
{
	...
	while ($StatRow = mysql_fetch_assoc($query))
	{
		...
		$parse['player_mes']      = "<a href=\"messages.php?mode=write&id=" . $UsrRow['id'] . "\"><img src=\"" . $dpath . "img/m.gif\" border=\"0\" title=\"Escribir un mensaje\" /></a>";
...
display(parsetemplate( gettemplate('stat/stat_body'), $parse ), "Estadísticas");
?>