							...
							foreach ($CombatCaps[$element]['sd'] as $c => $d) {
								if (isset($defender['def'][$c])) {
									if ($d > 0) {
										...
										$defender_n[$fleetID2][$c] -= floor(($amount * $d * (rand(80,100) / 100) / 2) * $defensePct[$fleetID2] * ($amount / $attackAmount[$fleetID]));
						...
						if ($amount > 0) {
							foreach ($CombatCaps[$element]['sd'] as $c => $d) {
								if (isset($attacker['detail'][$c])) {
									$d = $d / $defender['techs'][0] / $defender['techs'][1] * $attacker['techs'][2];
									$attacker_n[$fleetID][$c] -= floor(($amount * $d * rand(80,100) / 100 / 2) * $attackPct[$fleetID] * ($amount / $defenseAmount[$fleetID2]));
