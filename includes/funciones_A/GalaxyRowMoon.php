...
{
	global $user, $dpath, $HavePhalanx, $CurrentSystem, $CurrentGalaxy, $CanDestroy;
	...
	if ($GalaxyRowUser['id'] != $user['id'])
		if ($CanDestroy > 0)
			$MissionType9Link = "<a href=fleet.php?galaxy=".$Galaxy."&system=".$System."&planet=".$Planet."&planettype=".$PlanetType."&target_mission=9>Destruir</a>";
		else
			$MissionType9Link = "";
	elseif ($GalaxyRowUser['id'] == $user['id'])
		$MissionType9Link = "";
	...
	if ($GalaxyRow && $GalaxyRowPlanet["destruyed"] == 0 && $GalaxyRow["id_luna"] != 0)
	{
		...
		$Result .= "Luna ".$GalaxyRowPlanet["name"]." [".$Galaxy.":".$System.":".$Planet."]";
		$Result .= "</td>";
		$Result .= "</tr><tr>";
		$Result .= "<th width=80>";
		...
		$Result .= "<table>";
		$Result .= "<tr>";
		$Result .= "<td class=c colspan=2>Características</td>";
		$Result .= "</tr><tr>";
		$Result .= "<th>Diámetro</th>";
		$Result .= "<th>". number_format($GalaxyRowPlanet['diameter'], 0, '', '.') ."</th>";
		$Result .= "</tr><tr>";
		$Result .= "<th>Temperatura</th><th>". number_format($GalaxyRowPlanet['temp_min'], 0, '', '.') ."</th>";
		$Result .= "</tr><tr>";
